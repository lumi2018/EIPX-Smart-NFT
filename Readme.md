...
Finally, if both attributes asset and user are defined, the token states define if the asset and the owner or the asset and the user have authenticated each other or not (waitingForOwner, engagedWithOwner, waitingForUser and engagedWithUser). The flow chart in Figure 3 shows all the possible state changes. The states related to the owner are the same than in Figure 2. The difference is that, at the state “engagedWithOwner”, the token can be assigned to a user. At the states "engagedWithOwner", "waitingForUser" or "engagedWithUser", if a user is assigned, the token changes its state to "waitingForUser". Once the asset and the user authenticate each other, the state of the token is set to "engagedWithUser", and the user is able to use the asset because the operating mode of the asset allows it.
